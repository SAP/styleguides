...
link:https://sonarcloud.io/[SonarCloud] is a product for Continuous Code Quality online, totally free for open-source projects. It supports multiple languages, Abap serialized by AbapGIT being one of them. There are about 100 different rules, with examples and explainations, that can be turned on and off depending on your needs. SonarCloud can be used to seamelessly integrate Abap Code together with checks for other languages that a project might use.
