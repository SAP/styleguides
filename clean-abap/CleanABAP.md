...
Some further thoughts taken in part from Chapter 2 of Martin Fowler's book "Refactoring":
...
in ABAP a large proportion is likely to be database time. 
...
focus more on clean and well structured code during initial development, and use the profiler
to identify critical areas to spend effort on optimizing afterwards. 

In fact, we would argue that a clean code approach will have a net positive effect on performance 
because it is a more focused use of optimization effort, and it should be easier 
