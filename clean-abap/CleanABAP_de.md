...
- [How-to](#how-to)
   ...
   - [How-to: Automatische Prüfung](#how-to-automatische-prüfung)
   - [How-to: Weitere Leitfäden](#how-to-weitere-leitfäden)
   - [How-to: Kritik, Änderungsvorschläge](#how-to-kritik-änderungsvorschläge)
- [Namen](#namen)
   - [Aussagekräftige Namen verwenden](#aussagekräftige-namen-verwenden)
   - [Begriffe der Lösungsdomäne und Problemdomäne bevorzugen](#begriffe-der-lösungsdomäne-und-problemdomäne-bevorzugen)
   - [Plural verwenden](#plural-verwenden)
   - [Aussprechbare Namen verwenden](#aussprechbare-namen-verwenden)
   - [Abkürzungen vermeiden](#abkürzungen-vermeiden)
   - [Abkürzungen konsistent verwenden](#abkürzungen-konsistent-verwenden)
   - [Nomen für Klassen, Verben für Methoden](#nomen-für-klassen-verben-für-methoden)
   - [Füllwörter wie „Daten“, „Info“, „Objekt“ vermeiden](#füllwörter-wie-daten-info-objekt-vermeiden)
   - [Ein Wort pro Konzept wählen](#ein-wort-pro-konzept-wählen)
   - [Musternamen nur mit Absicht verwenden](#musternamen-nur-mit-absicht-verwenden)
   - [Codierungen vermeiden, insbes. Ungarische Notation und Präfixe](#codierungen-vermeiden-insbes-ungarische-notation-und-präfixe)
- [Sprache](#sprache)
   - [Vorsicht bei älteren ABAP-Releases](#vorsicht-bei-älteren-abap-releases)
...
- [Tabellen](#tabellen)
   ...
   - [Überflüssige Lesezugriffe auf Tabelle vermeiden](#überflüssige-lesezugriffe-auf-tabelle-vermeiden)
- [Strings](#strings)
...
- [Boolesche Ausdrücke](#boolesche-ausdrücke)
   ...
   - [ABAP_BOOL für boolesche Ausdrücke verwenden](#abap_bool-für-boolesche-ausdrücke-verwenden)
   - [ABAP_TRUE und ABAP_FALSE für Vergleiche verwenden](#abap_true-und-abap_false-für-vergleiche-verwenden)
   - [XSDBOOL für boolesche Variablen verwenden](#xsdbool-für-boolesche-variablen-verwenden)
- [Bedingungen](#bedingungen)
   - [Bedingungen nach Möglichkeit positiv definieren](#bedingungen-nach-möglichkeit-positiv-definieren)
...
- [IF](#if)
   ...
   - [Schachtelungstiefe so gering wie möglich halten](#schachtelungstiefe-so-gering-wie-möglich-halten)
- [Reguläre Ausdrücke](#reguläre-ausdrücke)
   - [Besser einfachere Methoden als reguläre Ausdrücke](#besser-einfachere-methoden-als-reguläre-ausdrücke)
   - [Besser Basisprüfungen als reguläre Ausdrücke](#besser-basisprüfungen-als-reguläre-ausdrücke)
   - [Komplexe reguläre Ausdrücke zusammensetzen](#komplexe-reguläre-ausdrücke-zusammensetzen)
- [Klassen](#klassen)
   ...
   - [Scope](#scope)
      ...
      - [Attribute und Methoden standardmäßig PRIVATE, nur im Bedarfsfall PROTECTED](#attribute-und-methoden-standardmäßig-private-nur-im-bedarfsfall-protected)
      - [Unveränderlichkeit anstelle von Gettern erwägen](#unveränderlichkeit-anstelle-von-gettern-erwägen)
      - [READ-ONLY sparsam verwenden](#read-only-sparsam-verwenden)
   - [Konstruktoren](#konstruktoren)
      - [Besser NEW als CREATE OBJECT](#besser-new-als-create-object)
      - [Bei globaler Klasse CREATE PRIVATE lassen Sie den CONSTRUCTOR öffentlich](#bei-globaler-klasse-create-private-lassen-sie-den-constructor-öffentlich)
      ...
      - [Aussagekräftige Namen bei mehreren Erstellungsmethoden verwenden](#aussagekräftige-namen-bei-mehreren-erstellungsmethoden-verwenden)
...
- [Methoden](#methoden)
   - [Aufrufe](#aufrufe)
      ...
      - [Optionales Schlüsselwort EXPORTING weglassen](#optionales-schlüsselwort-exporting-weglassen)
   ...
   - [Methoden: Objektorientierung](#methoden-objektorientierung)
      ...
      - [Öffentliche Instanzmethoden sollten Teil einer Schnittstelle sein](#öffentliche-instanzmethoden-sollten-teil-einer-schnittstelle-sein)
   - [Parameteranzahl](#parameteranzahl)
      - [So wenig IMPORTING-Parameter wie möglich, im Bestfall weniger als drei](#so-wenig-importing-parameter-wie-möglich-im-bestfall-weniger-als-drei)
      - [Besser Methoden aufteilen als OPTIONAL-Parameter hinzufügen](#besser-methoden-aufteilen-als-optional-parameter-hinzufügen)
   ...
   - [Parametertypen](#parametertypen)
      ...
      - [RETURNING von großen Tabellen ist in der Regel okay](#returning-von-großen-tabellen-ist-in-der-regel-okay)
   ...
   - [Parameterinitialisierung](#parameterinitialisierung)
      - [EXPORTING-Referenzparameter löschen oder überschreiben](#exporting-referenzparameter-löschen-oder-überschreiben)
         - [Vorsicht bei identischer Ein- und Ausgabe](#vorsicht-bei-identischer-ein--und-ausgabe)
      - [VALUE-Parameter nicht löschen](#value-parameter-nicht-löschen)
   - [Methodenrumpf](#methodenrumpf)
      - [Tue eine Sache, nur eine, und mache sie gut](#tue-eine-sache-nur-eine-und-mache-sie-gut)
      - [Glücklicher Pfad oder Fehlerbehebung, nicht Beides](#glücklicher-pfad-oder-fehlerbehandlung-aber-nicht-beides-zugleich)
...
- [Fehlerbehandlung](#fehlerbehandlung)
   ...
   - [Rückgabecodes](#rückgabecodes)
      - [Ausnahmen statt Rückgabecodes](#ausnahmen-statt-rückgabecodes)
      - [Alle Fehler abfangen](#alle-fehler-abfangen)
   - [Ausnahmen](#ausnahmen)
      - [Ausnahmen sind für Fehler gedacht, nicht für den Normalfall](#ausnahmen-sind-für-fehler-gedacht-nicht-für-den-normalfall)
      - [Klassenbasierte Ausnahmen verwenden](#klassenbasierte-ausnahmen-verwenden)
   - [Ausnahme absetzen](#ausnahme-absetzen)
      - [Eigene übergeordnete Klassen verwenden](#eigene-übergeordnete-klassen-verwenden)
      - [Nur einen Ausnahmetyp werfen](#nur-einen-ausnahmetyp-werfen)
      - [Übersichtlichere Fehlersituationen mit untergeordneten Klassen](#übersichtlichere-fehlersituationen-mit-untergeordneten-klassen)
      - [CX_STATIC_CHECK für behandelbare Ausnahmen absetzen](#cx_static_check-für-behandelbare-ausnahmen-absetzen)
      - [CX_NO_CHECK für gewöhnlich nicht behebbare Situationen absetzen](#cx_no_check-für-gewöhnlich-nicht-behebbare-situationen-absetzen)
      - [CX_DYNAMIC_CHECK für vermeidbare Ausnahmen absetzen](#cx_dynamic_check-für-vermeidbare-ausnahmen-absetzen)
      - [Dump für schwerwiegende, nicht behebbare Situationen absetzen](#dump-für-schwerwiegende-nicht-behebbare-situationen-absetzen)
   ...
   - [Ausnahmen abfangen](#ausnahmen-abfangen)
      - [Externe Ausnahmen umschließen, um das Eindringen in Ihren Code zu verhindern](#externe-ausnahmen-umschließen-um-das-eindringen-in-ihren-code-zu-verhindern)
- [Kommentare](#kommentare)
   ...
   - [Kommentare gehören vor die Anweisung, auf die sie sich beziehen](#kommentare-gehören-vor-die-anweisung-auf-die-sie-sich-beziehen)
   - [Code löschen, nicht kommentieren](#code-löschen-nicht-kommentieren)
   - [FIXME, TODO und XXX verwenden, und Ihre ID hinzufügen](#fixme-todo-und-xxx-verwenden-und-ihre-id-hinzufügen)
   ...
   - [ABAP Doc nur für öffentliche APIs](#abap-doc-nur-für-öffentliche-apis)
...
- [Formatierungen](#formatierungen)
   ...
   - [Vernünftige Zeilenlänge einhalten](#vernünftige-zeilenlänge-einhalten)
   ...
   - [Klammern am Zeilenende schließen](#klammern-am-zeilenende-schließen)
   ...
   - [Bei Zeilenumbruch Parameter unter dem Aufruf einrücken](#bei-zeilenumbruch-parameter-unter-dem-aufruf-einrücken)
   ...
   - [Einrücken und Tabulator verwenden](#einrücken-und-tabulator-verwenden)
   - [Inline-Deklarationen wie Methodenaufrufe einrücken](#inline-deklarationen-wie-methodenaufrufe-einrücken)
   - [Type-Klauseln nicht ausrichten](#type-klauseln-nicht-ausrichten)
- [Test](#test)
   - [Grundlagen](#grundlagen)
      - [Testbaren Code schreiben](#testbaren-code-schreiben)
      - [Ermöglichen Sie anderen, Test-Doubles zu verwenden](#ermöglichen-sie-anderen-test-doubles-zu-verwenden)
      - [Regeln für die Lesbarkeit](#regeln-für-die-lesbarkeit)
      - [Keine Kopien oder Testreports](#keine-kopien-oder-testreports)
      - [Nur PUBLIC-Anteile testen](#nur-public-anteile-testen)
      - [Nicht von der Code Coverage verrückt machen lassen](#nicht-von-der-code-coverage-verrückt-machen-lassen)
   - [Testklassen](#testklassen)
      ...
      - [Testklassen ausführen](#testklassen-ausführen)
   ...
   - [Injection](#injection)
      - [Abhängigkeitsumkehr zum Einbringen von Testattrappen verwenden](#abhängigkeitsumkehr-zum-einbringen-von-testattrappen-verwenden)
      - [ABAP-Testattrappe verwenden](#abap-testattrappe-verwenden)
      - [Von Test-Tools unterstützen lassen](#von-test-tools-unterstützen-lassen)
      - [Testseams als temporäre Behelfslösung verwenden](#testseams-als-temporäre-behelfslösung-verwenden)
      - [Mit LOCAL FRIENDS auf Abhängigkeitsumkehr-Konstruktor zugreifen](#mit-local-friends-auf-abhängigkeitsumkehr-konstruktor-zugreifen)
      ...
      - [Produktiven Code nicht zugunsten Testbarkeit ändern](#produktiven-code-nicht-zugunsten-testbarkeit-ändern)
      - [Keine Unterklassen zum Nachstellen von Methoden](#keine-unterklassen-zum-nachstellen-von-methoden)
      - [Nichts Unnötiges nachstellen](#nichts-unnötiges-nachstellen)
   ...
   - [Assertions](#Assertions)
      ...
      - [Qualität, nicht Inhalt validieren](#qualität-nicht-inhalt-validieren)
      - [FAIL zum Prüfen erwarteter Ausnahmen verwenden](#fail-zum-prüfen-erwarteter-ausnahmen-verwenden)
      ...
      - [Angepasste Assertions: Code verkürzen, Doppeltes vermeiden](#angepasste-Assertions-code-verkürzen-doppeltes-vermeiden)
